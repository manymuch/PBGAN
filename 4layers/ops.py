...
def _MySignGrad(op, grad):
  x = op.inputs[0]
  result = tf.add(tf.scalar_mul(0.5,tf.negative(tf.sign(tf.subtract(tf.abs(x),tf.ones(x.get_shape()))))),tf.scalar_mul(0.5,tf.ones(x.get_shape())))
  #print(result)
...
def bconv2d(input_, output_dim, 
  ...
  with tf.variable_scope(name):
    ...
    return conv
def de_conv2d(input_, output_dim, k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02,name="de_conv2d"):
  with tf.variable_scope(name):
    w = tf.get_variable('w', [k_h, k_w, 3,input_.get_shape()[-1]],initializer=tf.truncated_normal_initializer(stddev=stddev))
    biases =tf.negative(tf.get_variable('biases', [input_.get_shape()[-1]], initializer=tf.constant_initializer(0.0)))
    predeconv=tf.reshape(tf.nn.bias_add(input_, biases), input_.get_shape())
    conv = tf.nn.conv2d_transpose(predeconv, w, output_shape=output_dim,strides=[1, d_h, d_w, 1])

    return conv

def binconv2d(input_, output_dim, 
       k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02,
       name="binconv2d"):
  with tf.variable_scope(name):
    w = tf.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim],
...
def bdeconv2d(input_, output_shape,
       k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02,
       name="deconv2d", with_w=False):
  with tf.variable_scope(name):
    # filter : [height, width, output_channels, in_channels]
...
def bindeconv2d(input_, output_shape,
       k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02,
       name="bindeconv2d", with_w=False):
  with tf.variable_scope(name):
    # filter : [height, width, output_channels, in_channels]
    w = tf.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]],
              initializer=tf.random_normal_initializer(stddev=stddev))
...
def linear(input_, output_size, scope=None, stddev=0.02, bias_start=0.0, with_w=False):
  ...
  with tf.variable_scope(scope or "Linear"):
    ...
    if with_w:
      return tf.matmul(input_, matrix) , matrix , bias
    else:
      return tf.matmul(input_, matrix)

def nobias_linear(input_, output_size, scope=None, stddev=0.02, bias_start=0.0, with_w=False):
  shape = input_.get_shape().as_list()
