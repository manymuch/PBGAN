...
def conv2d_d2(input_, output_dim, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02,name="conv2d_d2"):
  with tf.variable_scope(name):
    w = tf.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim],initializer=tf.truncated_normal_initializer(stddev=stddev))
    conv = tf.nn.conv2d(input_, w, strides=[1, d_h, d_w, 1], padding='SAME')
    biases = tf.get_variable('biases', [output_dim], initializer=tf.constant_initializer(0.0))
...
def deconv2d_d1(input_, output_shape,
  ...
  with tf.variable_scope(name):
    ...
    if with_w:
      ...
      return deconv
def deconv2d_d2(input_, output_shape,k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02,name="deconv2d_d2", with_w=False):
  with tf.variable_scope(name):
    # filter : [height, width, output_channels, in_channels]
    w = tf.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]],
              initializer=tf.random_normal_initializer(stddev=stddev))
...
def bdeconv2d_d1(input_, output_shape,
       k_h=3, k_w=3, d_h=1, d_w=1, stddev=0.02,
       name="bdeconv2d_d1", with_w=False):
  with tf.variable_scope(name):
    # filter : [height, width, output_channels, in_channels]
    w = tf.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]],
              initializer=tf.random_normal_initializer(stddev=stddev))
...
def bdeconv2d_d2(input_, output_shape,
       k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02,
       name="bdeconv2d_d2", with_w=False):
  with tf.variable_scope(name):
    # filter : [height, width, output_channels, in_channels]
    w = tf.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]],
              initializer=tf.random_normal_initializer(stddev=stddev))
    ...
    if with_w:
      ...
      return deconv
def de_conv2d_d1(input_,output_dim, k_h=3, k_w=3, d_h=1, d_w=1, stddev=0.02,name="conv2d_d1"):
  with tf.variable_scope(name):
    w = tf.get_variable('w', [k_h, k_w, 3,input_.get_shape()[-1]],
              initializer=tf.truncated_normal_initializer(stddev=stddev))

    
    biases =tf.negative(tf.get_variable('biases', [input_.get_shape()[-1]], initializer=tf.constant_initializer(0.0)))

    #predeconv=tf.multiply(alpha,tf.reshape(tf.nn.bias_add(input_, biases), input_.get_shape()))
